Spree::StateChange.class_eval do
  ...
  def newgistics_status
    ...
    { paid: 'UPDATED', balance_due: 'ONHOLD', failed: 'ONHOLD',  }[next_state.to_sym] || ''
  end

  def newgistics_status_notes
    #status notes are only required by newgitics for transitions from any to ONHOLD
    { balance_due: 'Payment pending', failed: 'Failed payment',  }[next_state.to_sym] || ''
  end
end
